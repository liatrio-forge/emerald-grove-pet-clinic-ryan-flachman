...
| AC-3.a | `./mvnw test` exits 0 with no failures | `13-proofs/13-task-05-proofs.md` | command output | PASS |

## Definition of done

- [x] AC-1.a: `anthropic.api.key=${ANTHROPIC_API_KEY:}` in `application.properties`
- [x] AC-1.b: `anthropic.api.url=https://api.anthropic.com/v1/messages` in `application.properties`
- [x] AC-1.c: `anthropic.model=claude-haiku-4-5-20251001` in `application.properties`
- [x] AC-1.d: `AsyncConfigPropertiesTest` passes with all three `containsProperty` assertions `true`
- [x] AC-2.a: `AsyncConfig.java` exists in `system` package with `@Configuration @EnableAsync`
- [x] AC-2.b: Bean method `visitSummaryExecutor()` present and returns `ThreadPoolTaskExecutor`
- [x] AC-2.c: `AsyncConfigTest` asserts `corePoolSize == 2` — passing
- [x] AC-2.d: `AsyncConfigTest` asserts `maxPoolSize == 5` — passing
- [x] AC-2.e: `AsyncConfigTest` asserts `queueCapacity == 25` — passing
- [x] AC-2.f: `AsyncConfigTest` asserts rejection handler is `CallerRunsPolicy` — passing
- [x] AC-2.g: `AsyncConfigTest` asserts thread name prefix is `visitSummary-` — passing
- [x] AC-3.a: `./mvnw test` exits 0 with no test failures
...
- [x] Parent epic child-registry checkbox ticked (not applicable — no epic spec file).
